...
?>
	<div class="container">
		<div class="content">
			<h2>Data Tenaga Kerja &raquo; Input Nama Sponsor</h2>
			<hr />
			...
			//	$cek = mysqli_query($koneksi, "SELECT * FROM mahasiswa ORDER BY code ASC"); // query untuk memilih entri dengan nim terpilih
				$insert = mysqli_query($koneksi, "INSERT INTO nama_sps (nm_sps) VALUES('$nm_sps')") or die(mysqli_error()); // query untuk menambahkan data ke dalam database	
			if($insert){ // jika query insert berhasil dieksekusi
							echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Data Nama Sponsor Berhasil Di Simpan. <a href="data.php"><- Kembali</a></div>'; // maka tampilkan 'Data Mahasiswa Berhasil Di Simpan.'
						}else{ // jika query insert gagal dieksekusi
							echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Ups, Data Nama Sponsor Gagal Di simpan! <a href="data.php"><- Kembali</a></div>'; // maka tampilkan 'Ups, Data Mahasiswa Gagal Di simpan!'
						}
...
<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="col-sm-4">
					<input type="text" name="nm_sps" class="form-control" placeholder="Format : John Smith" required>
				</div>
				</div>
				...
				<div class="row">
					...
					if($filter){
						...
						$sql = mysqli_query($koneksi, "SELECT * FROM nama_sps ORDER BY id ASC"); // jika tidak ada filter maka tampilkan semua entri
					}
